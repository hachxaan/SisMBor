...
from OBTaller.models import UnidadCombustible, VwrCombustibleConsumoPromedioXDia, \
    VwrCombustibleConsumoPromedioXMes, VwrCombustibleConsumoPromedioXAno, VwrCombustibleConsumoAcumuladoXDia, \
    VwrCombustibleConsumoAcumuladoXMes, VwrCombustibleConsumoAcumuladoXAno, Unidad


class DashboardCombustibleConsumoGraficaView( TemplateView ):
    template_name = 'dashboard_combustible_consumo.html'
    ...
    def getDiario(self, id_unidad, ano_mes, tipo):
        ...
        try:
            ...
            data['unida_medida_combustible'] = Unidad.objects.get(id_unidad=id_unidad).get_unida_medida_combustible_display()
            data['data'] = rdata
            data['dias'] = dias
...
class DashboardCombustibleConsumoView(ValidaPerfilMixin, ValidaTemp1, ValidaTemp2, TemplateView):
    template_name = 'dashboard/dashboard_combustible_consumo.html'
        ...
        try:
            ...
            if action == 'searchdata':
                ...
                if vista == 'diario':
                    ...
                    ano = int(filtro[0])
                    mes = int(filtro[1])
                    for i in VwrCombustibleConsumoPromedioXDia.objects.filter(id_unidad=id_unidad,
                                                         fh_ticket__year=ano,
                                                         fh_ticket__month=mes
                                                         ):
                        item = i.toJSON()
                ...
                if vista == 'mensual':
                    ...
                    for i in VwrCombustibleConsumoPromedioXMes.objects.filter(id_unidad=id_unidad,
                ...
                if vista == 'anual':
                    ...
                    id_unidad = request.POST['id_unidad']
                    if VwrCombustibleConsumoPromedioXAno.objects.filter(id_unidad=id_unidad).exists():
                        data = []
                        for i in VwrCombustibleConsumoPromedioXAno.objects.filter(id_unidad=id_unidad):
                            item = i.toJSON()
                            data.append(item)
                    else:
                ...
                if vista == 'diario_acum':
                   ...
                   ano = int(filtro[0])
                   mes = int(filtro[1])
                   for i in VwrCombustibleConsumoAcumuladoXDia.objects.filter(id_unidad=id_unidad,
                                                                           fh_ticket__year=ano,
                                                                           fh_ticket__month=mes
                                                                            ):
                      item = i.toJSON()
                ...
                if vista == 'mensual_acum':
                   data = []
                   id_unidad = request.POST['id_unidad']
                   for i in VwrCombustibleConsumoAcumuladoXMes.objects.filter(id_unidad=id_unidad):
                        item = i.toJSON()
                        data.append(item)
                ...
                if vista == 'anual_acum':
                   data = []
                   id_unidad = request.POST['id_unidad']
                   for i in VwrCombustibleConsumoAcumuladoXAno.objects.filter(id_unidad=id_unidad):
                        item = i.toJSON()
                        data.append(item)
            if action == 'totales':
                data={}
    ...
    def get_context_data(self, **kwargs):
        ...
        context['unidad_combustible_consumo'] = 'active'
